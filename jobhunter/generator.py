...
class Generator:
    ...
    def _system_prompt(self, profile: Profile, materials: Materials) -> str:
        return (
            ...
            "- INCLUDE EVERY position from the candidate's CV — do not drop, merge or "
            "omit any employer, role or date. The tailored CV must contain their "
            "COMPLETE work history. Tailor by re-emphasising and rewording bullets "
            "within each role (and trimming weak bullets), never by deleting whole "
            "jobs. A tailored CV that leaves out real experience is a failure.\n"

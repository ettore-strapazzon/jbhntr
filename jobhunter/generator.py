...
class Generator:
    ...
    def _system_prompt(self, profile: Profile, materials: Materials) -> str:
        return (
            ...
            "fit the role.\n"
            "- HONEST HEADLINE. The name, headline/title line and summary describe who "
            "the candidate ACTUALLY is, based on their real roles. NEVER relabel them "
            "with the target job's title or a title they have not held (e.g. do not "
            "write 'Partner Operations Lead' in the headline just because that is the "
            "job) — that is misrepresentation. Keep their genuine professional "
            "identity and seniority; you may angle the wording toward the role, but "
            "the reader must never be misled about what they have done.\n"
            "- KEEP EACH EMPLOYER'S CONTEXT. If the source CV has a one-line "
            "description under a company (what the company is / does), keep that line "
            "for every company that has one.\n"
            ...
            "- Layout the CV so a plain-text reader can parse its structure. Line 1 = "
            "the candidate's NAME (keep their exact capitalisation). Line 2 = a short "
            "headline of their real identity. Line 3 = a contact line. Then, for each "
            "employer, in this exact line order: 'Employer - Location'; then (only if "
            "the source has one) a one-line company description ending in a full stop; "
            "then each position held there as 'Job Title (Dates)' with the dates ALWAYS "
            "in parentheses — e.g. 'Head of Strategy (Nov 2022 - Present)', never with "
            "a pipe or bare dash; then that position's bullet points starting '- '.\n\n"
